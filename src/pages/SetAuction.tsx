...
import { fetchGetAuctionById } from "../api/setAuction";
...
function SetAuction() {
  ...
  const queryOptionsAuctionPost: any = auctionId
    ...
    : {
        queryKey: ["getAuctionDataInUpdate"],
        queryFn: () =>
          fetchGetAuctionById("8f84be4e-d98c-4fea-a175-bb9a17cc627e"),
        queryOptions: { staleTime: Infinity, enabled: !!auctionId },
      };
  ...
  const [auctionPostData, auctionPostIsLoading] = useCustomQuery<Auction_post>(
    queryOptionsAuctionPost
  );
  const [bidsData, bidsIsLoading] = useCustomQuery<Bids>(queryOptionsBids);
  useEffect(() => {
    if (auctionId) {
      setUpdIsLoading(auctionPostIsLoading);
      const startDateState = dayjs(auctionPostData?.auction_start_date).format(
        "YYYY-MM-DD"
      );
      const endDateState = dayjs(auctionPostData?.auction_end_date).format(
        "YYYY-MM-DD"
      );
      const startTimeState = dayjs(auctionPostData?.auction_start_date).format(
        "HH:mm"
      );
      const endTimeState = dayjs(auctionPostData?.auction_end_date).format(
        "HH:mm"
      );
